...
import { products } from "../../(user)/product/Product";      
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="md:w-48 h-48 bg-gray-100 flex-shrink-0">
                      <img
                        ...
                        onError={(e) => {
                          e.target.src = 'https://via.placeholder.com/300x300?text=Coffee+Item';
                        }}
                      />
                    </div>
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <div className="flex items-center gap-1">
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-yellow-400">
                                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006..." clipRule="evenodd" />
                              </svg>
                              <span className="text-sm font-semibold">{item.rating}</span>
                            </div>
                          </div>
                          <h2 className="text-xl font-bold text-[#8B4513] mb-2">{item.name}</h2>
                          <p className="text-gray-600 text-sm mb-3 line-clamp-2">{item.description}</p>
                          <div className="flex items-center gap-4 mb-4">
                            <span className="text-lg font-bold text-[#8B4513]">NPR {item.price.toFixed(2)}</span>
                            <span className="text-sm text-gray-500">× {item.quantity}</span>
                            <span className="text-lg font-bold text-green-600">NPR {(item.price * item.quantity).toFixed(2)}</span>
                          </div>
                  ...
                  {cartWithDetails.map((item) => (
                    <div key={item.id} className="flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">NPR {item.price} × {item.quantity}</p>
                      </div>
                      <span className="font-bold text-[#8B4513]">NPR {(item.price * item.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
